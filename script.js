        ...
        document.querySelectorAll("#select__prdct").forEach((prd) => {
          for (let ind = 0; ind < 130; ind++) {
            if (database[ind]["Product Name"] == prd.value) {
              ...
              nutrients["phosphorus (mg)"].push(
                database[ind]["phosphorus (mg)"]
              );
              ...
              nutrients["potassium (mEq)"].push(
                database[ind]["potassium (mEq)"]
              );
            }
          }
        });
      ...
      function switchUnit(index, el_dyn) {
        ...
        document.getElementById(`unit_ml_${index}_${el_dyn}`).style.display =
          "block";
    ...
    <div class="unit__box">
        ...
        <div class="row__toggle--calc">
          ...
          html += `</select>
            <div class="combined__calc--toggle">
              <input type="number" class="input__calc--toggle" id="${name_select_inp}" />
              ${UNIT[index]}
            </div>
      ...
      function getVolumeProductsGrams() {
        ...
        document.querySelectorAll("#powder_formula_unit").forEach((el, ind) => {
          let unit = el.value;
          let val = document.querySelectorAll("#powder__formula__select")[ind]
            .value;
        ...
        console.log("//////////////////");
        document
          .querySelectorAll("#human_milk_fortifier_unit")
          .forEach((el, ind) => {
            let unit = el.value;
            ...
            if (unit === "gram") {
              checkNULLDBValue(
                ...
                document.querySelectorAll(
                  "#err-human__milk__fortifier__select"
                )[ind],
        ...
        document.querySelectorAll("#modular_unit").forEach((el, ind) => {
          ...
          if (unit === "scoop(s)") {
            ...
            volumn_amount_g["modular"].push(
              val * modularNutrients["g per tsp"][ind]
            );
          } else if (unit === "gram") {
      ...
      function getVolumeProductsMl() {
        ...
        for (let x = 0; x < counter; x++) {
          let unit = document.querySelector(
            `input[name="volume_1_${x}"]:checked`
          );
          if (!unit) {
        ...
        for (let x = 0; x < counter_2; x++) {
          let unit = document.querySelector(
            `input[name="volume_2_${x}"]:checked`
          );
          if (!unit) {
            console.log("Does not exist");
          } else {
            unit = unit.value;
            let val =
              document.querySelectorAll("#human__milk__select")[x].value * 1;
        ...
        for (let x = 0; x < counter_3; x++) {
          let unit = document.querySelector(
            `input[name="volume_3_${x}"]:checked`
          );
          if (!unit) {
            console.log("Does not exist");
          } else {
            unit = unit.value;
            let val =
              document.querySelectorAll("#ready__to__feed__select")[x].value *
              1;
        ...
        document
          .querySelectorAll("#powder__formula__select")
          .forEach((el, ind) => {
            volumn_amount_ml["powder"].push(
              volumePrdctGram["powder"][ind] *
                powerFormulaNutrients["displ ml"][ind]
            );
          });
      ...
      function getProductKcal() {
        ...
        for (let x = 0; x < volume["human_milk_fortifier"].length; x++) {
          let val =
            document.querySelectorAll("#human__milk__fortifier__select")[x]
              .value * 1;
      ...
      function getTheRecipe() {
        ...
        document.querySelectorAll("#select__prdct").forEach((prdc) => {
          ...
          let id =
            prdc.parentElement.childNodes[3].childNodes[3].childNodes[1].id
              .toString()
              .split("_");
      ...
      function updateAllNutrients() {
        ...
        for (let x = 0; x < volume_ml["liquid_concentrate"].length; x++) {
          ...
          nutrients["protein (g)"][counter] *=
            volume_ml["liquid_concentrate"][x];
        ...
        for (let x = 0; x < volume_g["human_milk_fortifier"].length; x++) {
          ...
          if (volume_g["human_milk_fortifier"][x] == 0) {
            ...
            nutrients["protein (g)"][counter] *=
              volume_g["human_milk_fortifier"][x];
            nutrients["fat (g)"][counter] *=
              volume_g["human_milk_fortifier"][x];
            nutrients["carb (g)"][counter] *=
              volume_g["human_milk_fortifier"][x];
            nutrients["vitamin A (IU)"][counter] *=
              volume_g["human_milk_fortifier"][x];
            nutrients["vitamin D (IU)"][counter] *=
            ...
            nutrients["phosphorus (mg)"][counter] *=
              volume_g["human_milk_fortifier"][x];
            nutrients["iron (mg)"][counter] *=
              volume_g["human_milk_fortifier"][x];
            nutrients["zinc (mg)"][counter] *=
              volume_g["human_milk_fortifier"][x];
            nutrients["sodium (mg)"][counter] *=
              volume_g["human_milk_fortifier"][x];
            nutrients["sodium (mEq)"][counter] *=
        ...
        for (let x = 0; x < volume_g["modular"].length; x++) {
          ...
          if (volume_g["modular"][x] == 0) {
            let val =
              document.querySelectorAll("#modular__select")[x].value * 1;
            nutrients["protein (g)"][counter] *= val;
            nutrients["fat (g)"][counter] *= val;
            nutrients["carb (g)"][counter] *= val;
            nutrients["vitamin A (IU)"][counter] *= val;
      ...
      <td data-label="Per 100 ml">${filterVal(
        ...
        if (checkForNULLValues()[0]) {
          document.querySelector(".error__msg--footer").innerHTML =
            checkForAddedProduct()[1];
          document.querySelector(".error__container").style.display = "block";
          document.getElementById("output__container--calc").style.display =
            "none";
